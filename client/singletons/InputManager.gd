...
# warning-ignore:return_value_discarded
	Signals.connect("toggle_option_bool", Callable(self, "toggle_options"))
...
func _input(event) -> void:
	if Global.in_game and not SceneHandler.transition:
		if event is InputEventKey:
			if event.pressed:
				#if chat focused
				if line_edit.get_viewport().gui_get_focus_owner() == line_edit:
					#chat focused + enter
...
func parse_keybind(input: InputEventKey) -> void:
	...
	if keybind_dict.has(input.as_text()):
		...
		if keybind == "stat":
			...
			Signals.emit_signal("toggle_questLog")
			AudioControl.play_audio("windowToggle")
		else:
			# skill
