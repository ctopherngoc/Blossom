...
func _ready():
	notification.add_theme_color_override("font_color", Color("ff0300"))
...
func _on_Button_pressed():
	...
	if password.text != confirm.text:
		...
		return
	elif password.text.is_empty() or email.text.is_empty() or confirm.text.is_empty():
...
func _on_HTTPRequest_request_completed(_result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
