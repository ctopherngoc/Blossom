...
func _ready():
	...
	Signals.connect("reset_default_keybind", Callable(self, "reset_default_keybind"))
	Signals.connect("key_in_use", Callable(self, "key_in_use"))
...
func _get_drag_data(_pos):
	if label.visible:
		...
		drag_label.position = -0.5 * drag_label.size
