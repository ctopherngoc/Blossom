...
func _on_Icon_mouse_entered():
	var skill_node = self.get_parent().get_parent().get_parent()
	var skill_tip = skill_info.instantiate()
	skill_tip.skill_id = skill_data.id
	var skills_origin = get_node("/root/GameWorld/UI/Control/Skills").global_position
	skill_tip.position.x = skills_origin.x - (skill_tip.size.x *  skill_tip.scale.x) + 5
	skill_tip.position.y = skills_origin.y
	add_child(skill_tip)
